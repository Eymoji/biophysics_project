...
##for i in range(-int(number_iteration/2), 0):
##    
...
##
##for i in range(1, int(number_iteration/2)+1):
##    
##    for j in range(number_receptor):
##
##        if ((number_of_absorption[:-i,j] == 1).any()) == True:
...
for i in range(-int(number_iteration/2), 0):
    ...
    for j in range(number_receptor):
        ...
        Correlations_receptor[i+int(number_iteration/2),j] = np.mean(number_of_absorption[-i:,j]*number_of_absorption[:i,j])
...
for j in range(number_receptor):
    ...
    Correlations_receptor[int(number_iteration/2),j] = (Correlations_receptor[int(number_iteration/2)-1,j]+Correlations_receptor[int(number_iteration/2+1),j])/2
...
popt, pcov = curve_fit(fit_correlations, Tau, Averaged_Correlations, p0 = np.array([0.2, 5]))
print(popt)

plt.figure(figsize=(16,16))
plt.plot(Tau, Averaged_Correlations)
plt.plot(Tau, fit_correlations(Tau, popt[0], popt[1]))
plt.xlabel("tau (s)")
plt.ylabel("Correlation")
plt.show()
